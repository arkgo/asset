...
func HttpForm(uuu string, data Map) string {
	...
	if err == nil {
		...
		body, err := ioutil.ReadAll(res.Body)
		if err == nil {
			//bingo.Logger.Debug("HttpForm", uuu, string(body))
			return string(body)
		}
	}
...
func HttpPost(url string, bodyType string, body io.Reader, args ...Map) string {
...
func HttpPostJson(url string, bodyType string, body io.Reader, args ...Map) Map {
	resBody := HttpPost(url, bodyType, body, args...)
	if resBody != "" {
		m := Map{}
		err := json.Unmarshal([]byte(resBody), &m)
		if err == nil {
			return m
		}
	}
...
func HttpPostXml(url string, bodyType string, body io.Reader, args ...Map) Map {
	resBody := HttpPost(url, bodyType, body, args...)
	fmt.Println("body", resBody)
	if resBody != "" {
		m := Map{}
		err := xml.Unmarshal([]byte(resBody), &m)
		fmt.Println("xml", err, m)
		if err == nil {
			return m
		}
	}
