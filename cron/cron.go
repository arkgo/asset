...
func (c *Cron) Schedule(schedule Schedule, cmd Job, args *Extra) string {
	...
	if len(c.IDchan) > 0 {
		...
		c.EntriesMap.Store(entry.ID, entry)
		c.EntryChan <- entry
		return entry.ID
	}
	return ""
...
func (c *Cron) Entries() sync.Map {
	return c.EntriesMap
}

// Entry returns a snapshot of the given entry, or nil if it couldn't be found.
func (c *Cron) Entry(id string) (*Entry, error) {
	if vv, ok := c.EntriesMap.Load(id); ok {
		if entry, ok := vv.(*Entry); ok {
			return entry, nil
		}
	}
...
func (c *Cron) Stop() {
	...
	c.EntriesMap.Range(func(k, v interface{}) bool {
		entryid := fmt.Sprintf("%v", k)
		c.RemoveEntry(entryid)
		c.EntriesMap.Delete(k)
		return true
	})
